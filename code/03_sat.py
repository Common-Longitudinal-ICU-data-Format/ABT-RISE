...
def _(df, pl):
    ...
    # Hierarchical failure reason: no overnight data 1st, then no IMV, then eligibility failures
    _day_flags = _day_flags.with_columns(
...
def _(df, failure_reason_df, pl, process_cohort):
    ...
    n_eligible_days = cohort_elig.filter(pl.col("on_vent_and_sedation") == 1)["hosp_id_icu_day"].n_unique()
...
def _(cohort_elig, np, pl, tqdm):
    ...
    vent_eligible = vent_eligible.with_columns(
        ...
        .then(
            (pl.col("min_sedation_dose") == 0).cast(pl.Int32).cum_sum().over("hosp_id_icu_day")
        )
        .otherwise(None)
        .cast(pl.Float64)
        .alias("rank_sedation"),
        ...
        .then(
            (pl.col("min_sedation_dose_non_ops") == 0).cast(pl.Int32).cum_sum().over("hosp_id_icu_day")
        )
        .otherwise(None)
        .cast(pl.Float64)
    ...
    for _key in tqdm(vent_eligible["hosp_id_icu_day"].unique().sort().to_list(), desc="Evaluating SAT flags"):
        ...
        for i in range(_n):
            ...
            if _flags:
                _row = {"_row_idx": int(_row_idxs[i]),
                        "_detected_dttm": int(_cur)}  # microseconds since epoch
    ...
    if _flag_results:
        _flag_df = pl.DataFrame(
            _flag_results,
            schema={"_row_idx": pl.UInt32, "_detected_dttm": pl.Int64,
                    **{c: pl.Float64 for c in _FLAG_COLS}},
        ).with_columns(
            *[
                pl.when(pl.col(c) == 1).then(
                    pl.from_epoch(pl.col("_detected_dttm"), time_unit="us")
                ).otherwise(None).alias(f"{c}_time")
                for c in _FLAG_COLS
            ],
        ).drop("_detected_dttm")
        ...
        vent_eligible = vent_eligible.with_columns(
            ...
            *[pl.lit(None).cast(pl.Datetime("us")).alias(f"{_f}_time") for _f in _FLAG_COLS],
        )

    vent_eligible = vent_eligible.drop("_row_idx")

    sat_flag_cols = _FLAG_COLS
    sat_time_cols = [f"{c}_time" for c in _FLAG_COLS]
    return sat_flag_cols, sat_time_cols, vent_eligible


@app.cell
def _(DATA_DIR, OUTPUT_PHI, Path, SOFA_BATCH_SIZE, TIMEZONE, cohort, cohort_elig, df, failure_flags_df, get_daily_sofa, pl, sat_flag_cols, sat_time_cols, vent_eligible):
    ...
    # Step 2: Eligible days only — SAT flag columns + time columns from cohort_with_delivery
    _flag_agg = (
        cohort_with_delivery.select(["hosp_id_icu_day"] + sat_flag_cols + sat_time_cols)
        .group_by("hosp_id_icu_day")
        .agg(
            [pl.col(c).max().alias(c) for c in sat_flag_cols]
            + [pl.col(c).min().alias(c) for c in sat_time_cols]
        )
    )
    ...
    df_grouped = df_grouped.join(_sed_stopped, on="hosp_id_icu_day", how="left")
    df_grouped = df_grouped.with_columns(
...
def _(
    ...
    table1 = pd.DataFrame({
        ...
        "SAT Eligible": [_el.get(c, "—") for c in _chars],
...
def _(
    ...
    consort_sat = [
        ...
        {"step": 2, "description": "Vent-days with IMV",
         ...
         "reason": "No IMV"},
        {"step": 3, "description": "Eligible days (≥4h: no paralytics, sedation, continuous)",
         "remaining": _n_elig,
    ...
    _det_df.select(["hosp_id_icu_day"] + _det_cols).write_csv(OUTPUT_SAT / f"detection_failures_{SITE_NAME}.csv")
...
def _(
    ...
    if "hospital_id" in cohort.columns:
        ...
        # Add site-level total_vent_days (pre-filter count of all IMV days)
